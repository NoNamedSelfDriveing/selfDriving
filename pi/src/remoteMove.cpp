...
void serviceFrame(int, int);
void openServer(int, struct sockaddr_in, char);
void setGPIO(void);

/*
 * void serviceFrame(port, camera NO.)
 *
 * It used in thread to call "system(servicFrame port camera NO.)"
 *
 */
...
int main(int argc, char* argv[])
{
    ...
    struct sockaddr_in msg_server;
    int frame_sock, msg_sock;

    openServer(msg_sock, msg_server, 'T');

    std::thread serviceCamera(serviceFrame, port, cam);     //port x, here port is x + 1  
...
void serviceFrame(int port, int cam)
{
    char* command;
    sprintf(command, "./serviceCam %d %d", port, cam);
...
{
    server_addr = {AF_INET, htons(port + 1), INADDR_ANY};
